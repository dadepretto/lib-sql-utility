...
declare
    ...
    @statementsToRecreateForeignKeys nvarchar(max),
    @tables cursor,
...
begin
    ...
    begin try
        ...
        execute [Utility].[DropForeignKeysReferencing]
            @Objects = @Objects,
            @StatementsToRecreate = @statementsToRecreateForeignKeys output,
            @IsDebug = @IsDebug;
        ...
        begin
            set @stmt = concat(
                N'truncate table ', @schemaName, N'.', @tableName, N' '
            );
        ...
        if isnull(@IsDebug, 0) = 1
        begin
            raiserror(@statementsToRecreateForeignKeys, 10, 1) with nowait;
        end;

        execute [sys].[sp_executesql]
            @stmt = @statementsToRecreateForeignKeys;
